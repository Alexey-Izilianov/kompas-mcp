# Сборка e2e_test: все исходники сервера + e2e_test.cs (-main:E2eTest)
$ErrorActionPreference = 'Stop'
$root = Split-Path $PSScriptRoot -Parent
$csc = "C:\Windows\Microsoft.NET\Framework64\v4.0.30319\csc.exe"
$sources = Get-ChildItem -Recurse (Join-Path $root "src") -Filter *.cs |
  ForEach-Object { $_.FullName }
$sources += (Join-Path $PSScriptRoot "e2e_test.cs")
...
$out = Join-Path $PSScriptRoot "e2e_test.exe"
$argList = @("-nologo","-codepage:65001","-platform:x64","-main:E2eTest","-out:$out") + $refs + $sources
& $csc @argList
